  ...
  const loadTaskData = async () => {
    try {
      ...
      if (taskData) {
        ...
        setRecordings(taskData.recordings ? taskData.recordings.map(uri => ({
          uri,
          duration: 0,
    ...
    const typeWriter = () => {
      if (i < selectedPhrase.length) {
        ...
        if (!title.trim()) {
          ...
          setDescription(prev => prev ? `${prev}. ${selectedPhrase}` : selectedPhrase);
        }
      }
    };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity 
            style={[styles.button, (!title.trim() || isSaving) && styles.buttonDisabled]} 
                  ...
                  <View key={index} style={styles.recordingItem}>
                    <View style={styles.recordingInfo}>
                      ...
                      <Text style={styles.recordingMeta}>
                        {formatRecordingTime(rec.duration)} • {new Date(rec.timestamp).toLocaleTimeString()}
                      </Text>
                    </View>
      ...
      >
        <View style={styles.confirmModalOverlay}>
          <View style={styles.confirmModalContent}>
            <Ionicons name="warning" size={50} color="#FF3B30" style={styles.warningIcon} />
