...
import { autoPriority, createTask, generateDescription } from "./services/api";
...
export default function AddTaskScreen() {
  ...
  const handleCopyDescription = () => {
    if (description.trim()) {
      // In React Native, we need to use Clipboard API
      // For now, show alert
  ...
  const actuallyAddTask = async () => {
    try {
      ...
      const apiResponse = await createTask({ input: title.trim() });
